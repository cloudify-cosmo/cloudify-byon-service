...
'''Scan mechanism's internal functions' unit tests'''
...
class SplitTests(unittest.TestCase):
    '''Scan mechanism's internal `_split` function's unit tests'''

    def __init__(self, *args, **kwargs):
        super(SplitTests, self).__init__(*args, **kwargs)
        self._patches = {}
        self._patches['fd_res_lim'] = mock.patch(
            'resource.getrlimit',
            mock.MagicMock(return_value=(20, None))
        )
        self._patches['o_fds'] = mock.patch(
            'cloudify_hostpool.backend.scan._open_file_descriptors',
            mock.MagicMock(return_value=2)
        )

    def setUp(self):
        for p in self._patches.itervalues():
            p.start()
...
class FillEndpointsTests(unittest.TestCase):
    '''Scan mechanism's `_fill_endpoints` function's unit tests'''
